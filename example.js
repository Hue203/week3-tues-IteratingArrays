...
function filter02() {
  console.log(
    inventors.filter(
      (item) => item.charAt(0) == item.charAt(item.indexOf(" ") + 1)
    )
  );
}
...
function forEachAss02() {
  ...
  transactions.forEach((item) => {
    if (item.type == "withdrawal") {
      result += `- You ${item.type} ${
        item.amount
      }. The new balance is ${(startAmount -= item.amount)}\n`;
    } else {
      result += `- You ${item.type} ${
        item.amount
      }. The new balance is ${(startAmount += item.amount)}\n`;
...
function reduceAss02() {
  let result = transactions.reduce(
    (sum, current) =>
...
function priceOfProduct() {
  shoppingCart.forEach(
    (current) => (current.sum = current.price * current.quantity)
  );
...
function addItemToCart(product, quantity) {
  ...
  if (currentProduct) {
    ...
    shoppingCart = [product, ...shoppingCart].sort((a, b) =>
      a.id < b.id ? -1 : 1
    );
  }
  console.log(shoppingCart);
